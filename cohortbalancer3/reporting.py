...
def create_visualizations(results: 'MatchResults', 
                         output_dir: str, 
                         prefix: str = "",
                         dpi: int = 300,
                         max_vars_balance: int = 15,
                         max_vars_dist: int = 8) -> Dict[str, str]:
    ...
    if results.propensity_scores is not None:
        try:
            ...
            fig_calibration.savefig(os.path.join(output_dir, calibration_filename), dpi=dpi)
            ...
            image_paths['propensity_comparison'] = os.path.join(output_dir, ps_comp_filename)
    ...
    try:
        ...
        warnings.warn(f"Could not create matched pairs distance visualization: {str(e)}")
    ...
    try:
        fig_cov_dist = plot_covariate_distributions(results, max_vars=max_vars_dist, figsize=(14, 16))
        ...
        image_paths['covariate_distributions'] = os.path.join(output_dir, cov_dist_filename)
    except Exception as e:
        warnings.warn(f"Could not create covariate distributions visualization: {str(e)}")
    ...
    if results.balance_statistics is not None:
        try:
            ...
            if len(top_vars) >= 2:
                ...
                image_paths['matched_pairs_scatter'] = os.path.join(output_dir, scatter_filename)
        except Exception as e:
            warnings.warn(f"Could not create matched pairs scatter visualization: {str(e)}")
    
    return image_paths


def export_tables(results: 'MatchResults', 
                 output_dir: str, 
    ...
    # 1. Matched data
    matched_data_path = os.path.join(output_dir, f"{prefix}matched_data_{timestamp}.csv")
    results.matched_data.to_csv(matched_data_path, index=True)
    table_paths['matched_data'] = matched_data_path
    
    # 2. Balance statistics
    if results.balance_statistics is not None:
        balance_path = os.path.join(output_dir, f"{prefix}balance_statistics_{timestamp}.csv")
        results.balance_statistics.to_csv(balance_path, index=False)
        table_paths['balance_statistics'] = balance_path
    
    # 3. Effect estimates
    if results.effect_estimates is not None:
        effects_path = os.path.join(output_dir, f"{prefix}effect_estimates_{timestamp}.csv")
    ...
    if results.rubin_statistics is not None:
        rubin_path = os.path.join(output_dir, f"{prefix}rubin_statistics_{timestamp}.csv")
    ...
    if results.balance_index is not None:
        balance_index_path = os.path.join(output_dir, f"{prefix}balance_index_{timestamp}.csv")
...
def generate_html_report(results: 'MatchResults', 
                        method_name: str, 
                        image_paths: Dict[str, str],
                        output_dir: str,
                        filename: str = "matching_report.html") -> str:
    ...
    if exact_match_cols:
        exact_match_text = f" Exact matching was enforced on: {', '.join(exact_match_cols)}."
    else:
        exact_match_text = ""
    ...
    covariate_section_number = (6 if has_propensity and has_treatment else 
                               5 if has_propensity or has_treatment else 4)
    
    # Figure numbering
    distance_figure_number = 6 if has_propensity else 4
    treatment_figure_number = 7 if has_propensity else 5
    covariate_figure_number = (9 if has_propensity and has_treatment else 
                              7 if has_propensity or has_treatment else 5)
    ...
    if has_propensity:
        ...
        propensity_section = propensity_template.format(
            propensity_comparison_img=relative_image_paths.get('propensity_comparison', ''),
    ...
    if 'matched_pairs_scatter' in relative_image_paths and 'scatter_template' in locals():
    ...
    if 'covariate_distributions' in relative_image_paths:
        ...
        covariate_distributions_section = covariate_template.format(
            covariate_distributions_img=relative_image_paths.get('covariate_distributions', ''),
...
def create_report(results: 'MatchResults', 
