...
def plot_propensity_distributions(
    ...
    ax.hist(
        control_ps, bins=bins, alpha=0.5, label="Control", density=True, color="orange"
    )

    # Add density curves
    if len(treatment_ps) > 1:
        kde_treatment = sns.kdeplot(
            treatment_ps, ax=ax, color="blue", label="_nolegend_"
        )
    if len(control_ps) > 1:
...
def plot_treatment_effects(
    ...
    has_ci = (
        "ci_lower" in effect_estimates.columns
        and "ci_upper" in effect_estimates.columns
    )
...
def plot_matching_summary(
    ...
    matched = [
        matched_counts["n_treatment_matched"],
        matched_counts["n_control_matched"],
    ]
    ...
    if matched_counts["n_treatment_matched"] > 0:
        ratio = (
            matched_counts["n_control_matched"] / matched_counts["n_treatment_matched"]
        )
...
def plot_covariate_distributions(
    ...
    for i, var in enumerate(top_vars):
        ...
        if is_binary:
            ...
            if len(orig_ctrl) > 1:
                sns.kdeplot(
                    orig_ctrl, ax=axes[i, 0], color="orange", label="_nolegend_"
                )
            if len(match_treat) > 1:
                sns.kdeplot(
                    match_treat, ax=axes[i, 1], color="blue", label="_nolegend_"
                )
            if len(match_ctrl) > 1:
                sns.kdeplot(
                    match_ctrl, ax=axes[i, 1], color="orange", label="_nolegend_"
                )
...
def plot_matched_pairs_scatter(
    ...
    treatment_col = (
        results.config.treatment_col
    )  # Keep this line if needed, though not directly used now
    ...
    if not match_id_pairs:
        ...
        fig, ax = plt.subplots(figsize=figsize)
        ax.text(
            0.5, 0.5, "No matched pairs found", ha="center", va="center", fontsize=14
        )
        ax.set_axis_off()
        return fig
    ...
    for t_id, c_id in match_id_pairs:
        ...
        if t_id not in original_data.index or c_id not in original_data.index:
            logger.warning(
                f"Skipping pair ({t_id}, {c_id}): ID not found in original data."
            )
            continue
...
def plot_match_groups(
    ...
    if not match_groups:
        ax.text(0.5, 0.5, "No matches found", ha="center", va="center", fontsize=14)
        ax.set_axis_off()
        return fig
