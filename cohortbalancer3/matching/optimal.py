...
def optimal_match(
    ...
    if finite_distances.size == 0:
        logger.warning("No finite distances available for matching. No pairs will be found.")
    ...
    if replace:
        ...
        n_copies = int(ratio)
        if n_copies > 1:
            logger.debug(f"Implementing {ratio}:1 matching by tiling control matrix {n_copies} times.")
            solver_distances = np.tile(distances, (1, n_copies))
        else:
            solver_distances = distances.copy()
        
        solver_distances[np.isinf(solver_distances) | np.isnan(solver_distances)] = max_finite * 1e6
        
        row_ind, col_ind = linear_sum_assignment(solver_distances)
        
        # Process matches, mapping tiled columns back to original
        for r, c in zip(row_ind, col_ind, strict=False):
            if np.isinf(distances[r, c % n_control]): continue
        ...
        for i in range(int(ratio)):
            ...
            new_matches_found = 0
            for r, c in zip(row_ind, col_ind, strict=False):
                if np.isinf(distances[r, c]) or used_controls[c]: continue
