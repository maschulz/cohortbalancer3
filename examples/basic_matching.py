...
def generate_synthetic_data(n_samples=1000, random_state=42):
    ...
    outcome = 1.2*x1 + 0.8*x2 - 0.5*x3 + 2*treatment + np.random.normal(0, 1, n_samples)
...
def main():
    ...
    naive_estimate = data[data['treatment'] == 1]['outcome'].mean() - \
    ...
    if results.balance_statistics is not None:
        ...
        print("\nBalance Statistics:")
        print(balance_df[['variable', 'smd_before', 'smd_after', 'var_ratio_before', 'var_ratio_after']]
              .sort_values('smd_before', ascending=False).head())
    ...
    print("Open this HTML file in a browser to see the complete visualization of results")
    print("Tip: If images aren't showing, check that the image files exist in the same directory as the HTML file")
