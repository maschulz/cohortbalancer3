...
class TestCaliperCalculation:
    ...
    def test_auto_caliper_propensity(self):
        ...
        np.random.seed(42)
        propensity_scores = np.random.beta(2, 5, 100)  # Beta distribution gives values between 0 and 1
    ...
    def test_auto_caliper_mahalanobis(self):
        ...
        np.random.seed(42)
        distance_matrix = np.random.gamma(2, 0.5, (20, 30))  # Gamma gives positive values
    ...
    def test_auto_caliper_euclidean(self):
        ...
        expected_caliper = np.percentile(distance_matrix, 90)  # Default is 90th percentile
