...
class TestPropensityEstimation:
    ...
    def sample_data(self):
        ...
        data = pd.DataFrame({
            ...
            'outcome': 3 + 2 * treatment + 0.5 * X1 + 0.3 * X2 + np.random.normal(0, 1, n)
        })
    ...
    def test_get_propensity_model_with_params(self):
        ...
        try:
            ...
            model = get_propensity_model(model_type="logistic", model_params=model_params)
            assert model is not None
    ...
    def test_get_propensity_model_with_random_state(self):
        ...
        try:
            random_state = 42
            model = get_propensity_model(model_type="logistic", random_state=random_state)
    ...
    def test_trim_by_propensity_common_support(self, sample_data, propensity_scores):
        ...
        assert all((score >= min_common and score <= max_common) for score in ps_trimmed)
    ...
    def test_trim_by_propensity_percentile(self, sample_data, propensity_scores):
        ...
        assert np.all((treated_in_trimmed >= treated_low) & (treated_in_trimmed <= treated_high))
        
        # Check that control units are within thresholds
        control_in_trimmed = ps_trimmed[treatment == 0]
        assert np.all((control_in_trimmed >= control_low) & (control_in_trimmed <= control_high))
    ...
    def test_assess_common_support(self, sample_data, propensity_scores):
        ...
        expected_keys = [
            "common_support_min", "common_support_max", "overlap_coefficient",
            "hist_treated", "hist_control", "bin_edges"
        ]
    ...
    def test_assess_propensity_overlap(self, sample_data):
        ...
        # Check that the function returns the expected keys
        expected_keys = [
            "ks_statistic", "ks_pvalue", "overlap_coefficient",
            "common_support_range", "treated_range", "control_range",
            "prop_in_common_support", "prop_treated_in_cs", "prop_control_in_cs"
        ]
        for key in expected_keys:
    ...
    def test_calculate_propensity_quality(self, sample_data):
        ...
        # Check that the function returns the expected keys
        expected_keys = [
            "ks_statistic_before", "ks_pvalue_before", "mean_diff_before",
            "ks_statistic_after", "ks_pvalue_after", "mean_diff_after",
    ...
    def test_estimate_propensity_scores(self, mock_cv_func, sample_data):
        ...
        mock_cv_result = {
            ...
            "cv_results": {"fold_aucs": [0.7, 0.75, 0.72], "mean_auc": 0.72, "std_auc": 0.02},
        ...
        try:
            ...
            # Check that the function returns the expected keys
            expected_keys = [
                "propensity_scores", "model", "cv_results", "model_type", "auc", "calibration"
            ]
            for key in expected_keys:
