...
from cohortbalancer3.matching.optimal import optimal_match, _apply_exact_matching
...
class TestOptimalMatching:
    ...
    def test_optimal_matching_with_caliper(self, sample_data):
        ...
        pairs, distances = optimal_match(data, distance_matrix, treat_mask, caliper=caliper)
    ...
    def test_optimal_matching_with_ratio(self, sample_data):
        ...
        pairs, distances = optimal_match(
            data, distance_matrix, treat_mask, ratio=2.0
        )
    ...
    def test_optimal_matching_complex_scenario(self):
        ...
        distance_matrix = np.array([
            ...
            [0.10, 0.22, 0.35, 0.45, 0.58, 0.69, 0.81, 0.92, 1.04, 0.11],  # Treatment 0 (binary=0)
            [0.17, 0.06, 0.13, 0.22, 0.36, 0.48, 0.60, 0.72, 0.84, 0.28],  # Treatment 1 (binary=1)
            [0.39, 0.28, 0.16, 0.05, 0.13, 0.26, 0.39, 0.50, 0.63, 0.50],  # Treatment 2 (binary=0)
            [0.61, 0.50, 0.38, 0.28, 0.15, 0.05, 0.17, 0.29, 0.42, 0.72],  # Treatment 3 (binary=1)
            [0.83, 0.72, 0.60, 0.50, 0.37, 0.27, 0.14, 0.06, 0.20, 0.94]   # Treatment 4 (binary=0)
        ])
        ...
        binary_0_treat_indices = [i for i, idx in enumerate(np.where(treat_mask)[0])
                                  if data.iloc[idx]["binary"] == 0]
        binary_0_control_indices = [i for i, idx in enumerate(np.where(~treat_mask)[0])
                                   if data.iloc[idx]["binary"] == 0]
        
        # Get all binary=1 treatment and control indices
        binary_1_treat_indices = [i for i, idx in enumerate(np.where(treat_mask)[0])
                                  if data.iloc[idx]["binary"] == 1]
        binary_1_control_indices = [i for i, idx in enumerate(np.where(~treat_mask)[0])
                                   if data.iloc[idx]["binary"] == 1]
