...
def test_one_to_one_matching_equal_counts():
    ...
    print(f"Extra control units in matched data but not in pairs: {len(extra_controls)}")
    if extra_controls:
...
def test_internal_flipping_correct_restored():
    ...
    expected_matched_indices = pd.Index(list(treat_ids_in_pairs) + list(control_ids_in_pairs))
