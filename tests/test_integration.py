...
class TestMatchingIntegration:
    ...
    def test_euclidean_optimal_match(self, sample_data):
        ...
        assert all(isinstance(control_idx, (int, np.integer)) for controls in pairs.values() for control_idx in controls)
    ...
    def test_propensity_score_matching(self, sample_data):
        ...
        for treat_idx, control_idxs in pairs_optimal.items():
            for control_idx in control_idxs:
                actual_dist = distance_matrix[treat_idx, control_idx]
                assert abs(actual_dist - next(d for d in distances_optimal if np.isclose(d, actual_dist))) < 1e-10
                
        for treat_idx, control_idxs in pairs_greedy.items():
            for control_idx in control_idxs:
                actual_dist = distance_matrix[treat_idx, control_idx]
                assert abs(actual_dist - next(d for d in distances_greedy if np.isclose(d, actual_dist))) < 1e-10 