...
def generate_data(index_values: List, n_samples: int = 100, 
                 random_state: int = 42) -> DataFrame:
    ...
    if len(index_values) != n_samples:
        raise ValueError(f"Index length ({len(index_values)}) must match data length ({n_samples})")
...
def test_small_to_large_matching():
    ...
    n_samples = 100
    indices = sorted(np.random.RandomState(123).choice(range(1000, 10000), size=n_samples, replace=False))
