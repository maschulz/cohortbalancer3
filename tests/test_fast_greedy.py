...
class TestFastGreedyMatching:
    ...
    def sample_data(self):
        ...
        data['propensity'] = 1 / (1 + np.exp(-(0.5 * data['x1'] + 0.1 * data['x2'] - 2 * (data['treatment'] - 0.5))))
    ...
    def test_basic_fast_greedy_matching(self, sample_data):
        ...
        pairs, distances = fast_greedy_match(sample_data, treat_mask, propensity_scores, config)
    ...
    def test_complex_caliper_application(self):
        ...
        data = pd.DataFrame({
            ...
            'x2':        [5,  6,  15],
            'propensity':[0.5, 0.7, 0.51] # C0 has a large PS diff, C1 has a small one
        }, index=['T0', 'C0', 'C1'])
    ...
    def test_fast_greedy_complex_scenario(self):
        ...
        pairs, distances = fast_greedy_match(data, treat_mask, propensity_scores, config)
        ...
        for cid in result_pairs['T0']:
            ...
            assert abs(data.loc[cid, 'propensity'] - data.loc['T0', 'propensity']) <= 0.15 # Caliper
        ...
        for cid in result_pairs['T1']:
            ...
            assert abs(data.loc[cid, 'propensity'] - data.loc['T1', 'propensity']) <= 0.15 # Caliper
