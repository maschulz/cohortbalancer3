...
from cohortbalancer3.matching.distances import (
    ...
    _standardize_data,
    _calculate_propensity_distances,
)
...
class TestDistanceCalculations:
    ...
    def test_euclidean_distance_with_standardization(self, sample_data):
        ...
        for i in range(3):
            for j in range(4):
                expected[i, j] = np.sqrt(np.sum((X_treat_std[i] - X_control_std[j])**2))
    ...
    def test_euclidean_distance_with_weights(self, sample_data):
        ...
        for i in range(3):
            for j in range(4):
                expected[i, j] = np.sqrt(np.sum((X_treat_weighted[i] - X_control_weighted[j])**2))
