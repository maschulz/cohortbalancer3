...
def test_integer_participant_ids():
    ...
    participant_ids = sorted(rng.choice(range(1000, 10000), size=n_samples, replace=False))
...
def test_original_and_matched_data_preservation():
    ...
    match_pairs_df = results.get_match_pairs()
    all_matched_ids = set(match_pairs_df['treatment_id']).union(set(match_pairs_df['control_id']))
