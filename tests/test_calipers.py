"""
Dedicated test suite for caliper functionality in CohortBalancer3.
...
class TestCaliperFunctionality:
    ...
    def test_propensity_caliper_excludes_bad_match(self, caliper_test_data, base_config):
        """
        ...
        matcher = Matcher(caliper_test_data, config)
        matcher.match()
    ...
    def test_covariate_specific_caliper(self, caliper_test_data, base_config):
        ...
        # The age difference between T0 (50) and C_Far_In (70) is 20.
        # This pair should be excluded, even though its propensity score is close.
        # The match should be with C_Close_In (age diff=1) or C_Close_Out (age diff=2).
        pairs = results.get_match_pairs()
        assert len(pairs) == 1
        assert pairs.iloc[0]['treatment_id'] == 'T0'
        assert pairs.iloc[0]['control_id'] != 'C_Far_In'
        assert abs(caliper_test_data.loc['T0', 'age'] - caliper_test_data.loc[pairs.iloc[0]['control_id'], 'age']) < 5

    def test_auto_caliper_with_mahalanobis(self, caliper_test_data, base_config):
        """Test that the 'auto' caliper for Mahalanobis distance is calculated and applied."""
        config = copy_config_with_updates(
            base_config,
            distance_method='mahalanobis',
            caliper_method='mahalanobis',
            caliper_value='auto',
            caliper_scale=0.05  # p-value
        )
        # With k=2 covariates, the 95% critical value is ~2.45.
        # All pairs are within this distance, so all should be considered.
        # The closest Mahalanobis match should be C_Close_In.
        ...
        assert len(pairs) == 1
        assert pairs.iloc[0]['treatment_id'] == 'T0'
        # The closest Mahalanobis distance should be to 'C_Close_In'
        assert pairs.iloc[0]['control_id'] == 'C_Close_In'
    ...
    def test_caliper_works_across_all_match_methods(self, caliper_test_data, base_config, match_method):
