...
class TestMatcher:
    ...
    def test_match_basic(self, sample_data, basic_config):
        ...
        assert len(results.match_pairs) == (results.matched_data['treatment'] == 1).sum()
    ...
    def test_match_with_exact_matching(self, sample_data, basic_config):
        ...
        for _, row in matched_pairs.iterrows():
            ...
            assert matched_data.loc[treat_idx, 'binary_var'] == matched_data.loc[control_idx, 'binary_var']
    ...
    def test_match_with_replacement(self, sample_data, basic_config):
        ...
        treat_X1_median = modified_data.loc[modified_data['treatment'] == 1, 'X1'].median()
        treat_X2_median = modified_data.loc[modified_data['treatment'] == 1, 'X2'].median()
        modified_data.loc[ideal_control_indices, 'X1'] = treat_X1_median + np.random.normal(0, 0.1, len(ideal_control_indices))
        modified_data.loc[ideal_control_indices, 'X2'] = treat_X2_median + np.random.normal(0, 0.1, len(ideal_control_indices))
        ...
        assert len(control_indices) >= len(unique_controls), "No controls were reused with replacement enabled"
    ...
    def test_match_with_direction_flipping(self, sample_data, basic_config):
        ...
        flip_indices = np.random.choice(flipped_data[control_mask].index, n_to_flip, replace=False)
        flipped_data.loc[flip_indices, 'treatment'] = 1
        
        # Verify we now have more treatment than control
        assert (flipped_data['treatment'] == 1).sum() > (flipped_data['treatment'] == 0).sum()
    ...
    def test_match_with_balance_calculation(self, sample_data, basic_config):
        ...
        assert results.balance_index["mean_smd_after"] <= results.balance_index["mean_smd_before"]
    ...
    def test_match_with_treatment_effect(self, sample_data, basic_config):
        ...
        assert 1.0 <= effect <= 3.0, f"Effect estimate {effect} is far from true effect of 2.0"
    ...
    def test_matcher_with_trimming(self, sample_data, basic_config):
        ...
        assert original_size - matched_size > min_group_size * 0.05  # At least 5% trimming
    ...
    @pytest.mark.parametrize("distance_method", ["euclidean", "mahalanobis", "propensity", "logit"])
    def test_different_distance_methods(self, sample_data, basic_config, distance_method):
    ...
    def test_optimal_matching_called_correctly(self, mock_optimal, sample_data, basic_config):
    ...
    def test_greedy_matching_called_correctly(self, mock_greedy, sample_data, basic_config):
    ...
    def test_edge_case_no_matches(self, sample_data, basic_config):
        ...
        config = copy_config_with_updates(basic_config,
                                         caliper=0.1)  # Very strict caliper
